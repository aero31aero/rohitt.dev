...
var Constructor = function (manager) {
    ...
    var generateGetAtForShaderBecauseGLSLisDumbAF = function (length) {
        ...
        for (var i = 0; i < length; i++) {
            string +=
                '\tif(index == ' + i + ') return timeDomainData[' + i + '];\n';
        }
        string += '}\n';
        return string;
    };
    ...
    var getSmoothArray = function (input) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            maxArray[i] =
                Math.max(
                    (maxArray[i] * (100 - percent)) / 100,
                    (avgArray[i] * (100 + percent * 5)) / 100
                ) *
    ...
    var make = function () {
        ...
        Logo.position.y =
            KHALIFA.building.position.y + KHALIFA.height / 2 + MINWIDTH / 9;
        ...
        scene.add(central);
        var upperBevel = new THREE.Mesh(
            geometries.centralBevel,
            materials.buildings
        );
        upperBevel.position.set(-100, 70 + CENTRAL.height / 2, -100);
        var lowerBevel = new THREE.Mesh(
            geometries.centralBevel,
            materials.buildings
        );
        ...
        world.update = function () {
            ...
            materials.buildings.uniforms.intensity.value =
                0.75 + camera.position.y * 0.00001;
            var audioDataArray = getSmoothArray(
                Array.from(audio.dataArray.slice(0, 128))
            );
            console.log(audioDataArray);
            materials.buildings.uniforms.timeDomainData.value = audioDataArray;
            materials.ground.uniforms.intensity.value =
                1 + camera.position.y * 0.00006;
            materials.ground.uniforms.camera.value = camera.position;

            ctx.drawImage(
                inputs.video,
                screen.width / 2 - (screen.height * 16) / 18,
                0,
                (screen.height * 16) / 9,
                screen.height
            );
    ...
    var initGeometries = function () {
        ...
        geometries.logo = new THREE.PlaneBufferGeometry(
            MINWIDTH,
            MINWIDTH / 4,
            1,
            1
        );
        geometries.ground = new THREE.PlaneBufferGeometry(
            TILE_LENGTH + TILE_SPACING,
            TILE_LENGTH + TILE_SPACING,
            1,
            1
        );
        geometries.screen = new THREE.PlaneBufferGeometry(
            MINWIDTH,
            MINWIDTH,
            1,
            1
        );
        geometries.screenbase = new THREE.BoxBufferGeometry(
            MINWIDTH + 5,
            10,
            10
        );
        geometries.stage = new THREE.CylinderGeometry(
            TILE_SPACING,
            0,
            MINHEIGHT,
            4
        ...
        geometries.lamppost = new THREE.CylinderGeometry(
            LAMPPOST.thickness,
            LAMPPOST.thickness,
            LAMPPOST.height
        );
        ...
        geometries.central = new THREE.CylinderGeometry(
            CENTRAL.radius,
            CENTRAL.radius,
            CENTRAL.height,
            CENTRAL.res,
            1,
            true,
            (5 * Math.PI) / 2 + CENTRAL.sector,
            (5 * Math.PI) / 2 - CENTRAL.sector
        );
        geometries.centralBevel = new THREE.CylinderGeometry(
            CENTRAL.radius + 5,
            CENTRAL.radius + 5,
            5,
            CENTRAL.res,
    ...
    var initMaterials = function () {
        ...
        materials.buildings = new THREE.ShaderMaterial({
            ...
            fragmentShader: $('#buildings-frag')
                .text()
                .replace(
                    '$insertUglyAssGetAtHere',
                    generateGetAtForShaderBecauseGLSLisDumbAF(128)
                ),
        });
    ...
    var initTextures = function () {
        ...
        const logo_texture = require('./buildings/textures/logo.png');
        const star_texture = require('./buildings/textures/star.png');
        const lamp_texture = require('./buildings/textures/lamp.png');

        textures.stars = loader.load(`/${star_texture.default}`);
        textures.logo = loader.load(`/${logo_texture.default}`);
        textures.lamp = loader.load(`/${lamp_texture.default}`);
        textures.lamp.wrapS = textures.lamp.wrapT = THREE.RepeatWrapping;

        textures.halftoneMap = loader.load(
            '/static/buildings/textures/halftoneBlur.jpg'
        );
        textures.halftoneMap.wrapS = textures.halftoneMap.wrapT =
    ...
    var initCity = function () {
        city.push(
            getTile(new THREE.Vector3(0, 0, 0), new THREE.Vector2(-1, -1), true)
        );
        city.push(
            getTile(
                new THREE.Vector3(TILE_SPACING, 0, 0),
        ...
        city.push(
            getTile(
                new THREE.Vector3(-TILE_LENGTH - TILE_SPACING, 0, 0),
                new THREE.Vector2(-1, -1),
                true
            )
        );
        city.push(
            getTile(
                new THREE.Vector3(0, 0, TILE_SPACING),
                new THREE.Vector2(-1, 1),
                true
            )
        );
        city.push(
            getTile(
                new THREE.Vector3(0, 0, -TILE_LENGTH - TILE_SPACING),
                new THREE.Vector2(-1, -1),
                true
            )
        );
        city.push(
            getTile(
                new THREE.Vector3(TILE_SPACING, 0, -TILE_LENGTH - TILE_SPACING),
                new THREE.Vector2(1, -1),
                true
            )
        );
        city.push(
            getTile(
                new THREE.Vector3(-TILE_LENGTH - TILE_SPACING, 0, TILE_SPACING),
                new THREE.Vector2(-1, 1),
                true
            )
        );
        city.push(
            getTile(
                new THREE.Vector3(TILE_SPACING, 0, TILE_SPACING),
                new THREE.Vector2(1, 1),
                true
            )
        );
    ...
    var getTile = function (origin, direction, isDeveloped) {
        ...
        if (isDeveloped) {
            for (let i = 0; i < LAYERS + 2; i++) {
                insertTrack(
                    TILE_LENGTH,
                    direction.x * (MINWIDTH / 2 + 2 * i * MINWIDTH),
                    MINWIDTH,
                    MINLENGTH,
                    MINHEIGHT,
                    50 + MINWIDTH * i * 2,
                ...
                insertTrack(
                    TILE_LENGTH,
                    direction.y * (MINWIDTH / 2 + 2 * i * MINWIDTH),
                    MINWIDTH,
                    MINLENGTH,
                    MINHEIGHT,
                    50 + MINWIDTH * i * 2,
    ...
    var insertTrack = function (
        ...
        var makeBuilding = function (length, height) {
            ...
            var building = new THREE.Mesh(
                box,
                new THREE.MeshPhongMaterial(0x3555ee)
            );
            building.castShadow = true;
            building.receiveShadow = true;
            building.position[secondDirection] =
                origin[secondDirection] + trackcenter;
            building.position.y = origin.y + height / 2;
            building.position[mainDirection] =
                origin[mainDirection] + direction * (trackdone + length / 2);
            return building;
        };
        var mesh;
        while (tracklength - trackdone > 45) {
            ...
            var distance = Math.pow(
                Math.pow(trackcenter + TILE_LENGTH / 2, 10) +
                    Math.pow(trackdone - tracklength / 2, 10),
                0.1
            );
            var height = Math.floor(
                minheight +
                    Math.random() *
                        Math.pow(1.083712, (TILE_LENGTH / 2 - distance) / 9) +
                    Math.random() * 70
            );
            trackdone += length + 10;
                ...
                if (targets.animating.hasOwnProperty(id)) {
                    ...
                    if (
                        Math.abs(
                            ...
                            Math.abs(animation.through - animation.to) <
                        0.1
                    ) {
                    ...
                    animation.through +=
                        (animation.to - animation.through) / ANIMATING_FACTOR;
                    var diff =
                        (animation.through -
    ...
    var initTimeline = function () {
        ...
        timeline.prev = function () {
            timeline.moveto(
                (timeline.now - 1 + timeline.keyframes.length) %
        ...
        timeline.keyframes.push({
            position: {
                to: KHALIFA.building.position
                    .clone()
                    .add(new THREE.Vector3(200, KHALIFA.height / 2 + 20, 200)),
            },
            lookAt: {
                to: KHALIFA.building.position
                    .clone()
                    .add(new THREE.Vector3(0, KHALIFA.height / 2, 0)),
            },
        });
        timeline.keyframes.push({
            position: {
        ...
        timeline.keyframes.push({
            position: {
                to: new THREE.Vector3(
                    -500 - 1.5 * (TILE_LENGTH + TILE_SPACING),
                    0,
                    -150 - 1.5 * (TILE_LENGTH + TILE_SPACING)
                ),
                through: new THREE.Vector3(
                    -1500 - (3 * TILE_LENGTH + TILE_SPACING),
                    2000,
                    -1500 - (TILE_LENGTH + TILE_SPACING)
                ),
            },
            lookAt: {
    ...
    window.addEventListener(
        ...
        function (e) {
            if (init == true) return;
            if ([40].indexOf(e.keyCode || e.which) != -1) timeline.next();
            if ([38].indexOf(e.keyCode || e.which) != -1) timeline.prev();
        },
        false
    );
    ...
    renderer.domElement.addEventListener(
        ...
        function (e) {
            ...
            var lineOfSight = frame.lookAt.to
                .clone()
                .add(frame.position.to.clone().negate());
            camera.right = lineOfSight.clone().cross(camera.up).normalize();
            var position = frame.lookAt.to
                .clone()
                .add(
                    camera.right
                        .clone()
                        .multiplyScalar(lineOfSight.length() * xdiff)
                )
                .add(
                    camera.up
                        .clone()
                        .multiplyScalar(-lineOfSight.length() * ydiff)
                );
...
$(document).ready(() => {
    ...
    document.getElementsByClassName(
        'immersive-wrapper'
    )[0].onmousewheel = onMousewheel;
    document.getElementsByClassName(
        'immersive-wrapper'
    )[0].ondommousescroll = onMousewheel;
